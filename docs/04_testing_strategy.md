...
2. **Eventos cercanos suman intensidad**
   - Given dos eventos en celdas adyacentes con misma categoría.
   - When se agrega el heatmap.
   - Then el score resultante refleja la suma ponderada y aparece como hotspot principal.
3. **Cambio de categorías altera el heatmap**
   - Given una mezcla de conciertos y mercados.
   - When se filtra por `categories=concierto`.
   - Then sólo las celdas asociadas a conciertos se devuelven, reduciendo el número de hotspots.

## 4. Herramientas
- `pytest` como runner general y para fixtures del dominio.
